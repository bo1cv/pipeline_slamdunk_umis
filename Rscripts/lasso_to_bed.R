...
final_residual_table<- final_residual_table %>%
  inner_join(stability %>% select(chr, start, end, transcript_id, strand))
...
final_residual_table %>% arrange(halflife) %>% head(n = number) %>%
  ...
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","halflife_lowstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")
final_residual_table %>% arrange(halflife) %>% tail(n = number) %>%
  bind_cols(rep(".", n = nrow(final_residual_table))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","halflife_highstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")

residual_table_hep_all %>% arrange(halflife) %>% head(n = number) %>%
  bind_cols(rep(".", n = nrow(residual_table_hep_all))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","halflife_lowstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")
residual_table_hep_all %>% arrange(halflife) %>% tail(n = number) %>%
  bind_cols(rep(".", n = nrow(residual_table_hep_all))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","halflife_highstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")
...
final_residual_table %>% arrange(Residuals) %>% head(n = number) %>%
  bind_cols(rep(".", n = nrow(final_residual_table))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","residual_lowstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")
final_residual_table %>% arrange(Residuals) %>% tail(n = number) %>%
  bind_cols(rep(".", n = nrow(final_residual_table))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","residual_highstab.bed"), col.names = F, quote = F, row.names = F, sep = "\t")

#Background
final_residual_table %>% arrange(halflife) %>%
  bind_cols(rep(".", n = nrow(final_residual_table))) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table(paste0(outdir, "/bed.dir/","background.bed"), col.names = F, quote = F, row.names = F, sep = "\t")

#Fire INPUTS
final_residual_table %>% select(transcript_id, halflife) %>%
  write.table("input_fire_halflife.txt", row.names = F, quote = F, sep = "\t")
final_residual_table %>% select(transcript_id, Residuals) %>%
  write.table("input_fire_residuals.txt", row.names = F, quote = F, sep = "\t")

final_residual_table %>%
  bind_cols(rep(".", n = nrow(final_residual_table))) %>%
  filter(length < 10000 & length > 6) %>%
  select(chr, start, end, transcript_id, `...10`,strand) %>%
  write.table("input_fire.bed", row.names = F, quote = F, sep = "\t", col.names = F)
...
colnames(all_residuals)[2] <- "halflife"
all_residuals$LogHalflife <- log(all_residuals$halflife)
...
all_residuals <- all_residuals %>% inner_join(stability_all %>% select(chr, start, end, transcript_id, strand))
