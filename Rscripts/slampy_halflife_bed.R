...
head(data_samples)
filter_data <- lapply(data_samples, subset, ReadsCPM >= arguments$cpm_cutoff)
filter_data_ConvRate <- lapply(filter_data, subset, select = c("Chromosome","Start","End","Name","Length","Strand", "ConversionRate"))
filter_data_ConvRate <- filter_data_ConvRate %>% 
  imap(.x = ., ~ set_names(.x, c("Chromosome","Start","End","Name","Length","Strand", .y))) %>%
  purrr::reduce(full_join, by = c("Chromosome","Start","End","Name","Length","Strand"))
head(filter_data_ConvRate)
filter_data_ConvRate[,6]  <- unite(filter_data_ConvRate[,1:6] , "Coordinate", sep = "_") 
filter_data_ConvRate <- filter_data_ConvRate[,6:15]
row.names(filter_data_ConvRate) <- filter_data_ConvRate$Strand
filter_data_ConvRate$Strand <- NULL
head(filter_data_ConvRate)
filter_data_ConvRate <- na.omit(filter_data_ConvRate)
ids <- row.names(filter_data_ConvRate)
...
if (arguments$bg == 1) {
  ...
  head(bg_convrate)
  bg_convrate <- bg_convrate[bg_convrate$Coordinate  %in% ids,]
...
best_half2 <- separate(best_half, X1, sep = "_", into = c("Chromosome","Start", "End", "Name", "Length", "Strand"),remove = FALSE)
best_half2$X1 <- NULL
names(best_half2) <- c("Chromosome","Start", "End", "Name", "Length", "Strand", "halflife","model")
best_half2$halflife <- as.numeric(best_half2$halflife)
head(best_half)
...
random_half <- matrix(nrow = nrow(ConvRate_rmbg_norm), ncol = 0)
random_half$id <- row.names(ConvRate_rmbg_norm)
for (y in 1:1000) {
  tmp_bs <- str_extract(colnames(dataframe_boostrap), regex("(?<=-).+(?=h)")) %>% as.numeric() %>% unique()
  random_data <- matrix(nrow = nrow(dataframe_boostrap), ncol = 0, byrow = T)
  rownames(random_data) <- rownames(dataframe_boostrap)
  for (i in tmp_bs) {
    #Getting stats on data separate timepoints
    std <- dplyr::select(dataframe_boostrap, matches(paste0("-",i,"h-"))) %>% apply(1,sd)
    mean <- dplyr::select(dataframe_boostrap, matches(paste0("-",i,"h-"))) %>% apply(1,mean)
    tmp_stats <- data.frame(rep(3, length(std)),mean, std)
    #Generating random data
    random_tmp <- apply(tmp_stats, 1, function(x) rnorm(x[1], mean=x[2], sd=x[3])) %>% t() %>% as.data.frame()
    colnames(random_tmp) <- paste0(rep(i, 3), "h")
...
# # Percentile bs CI  -------------------------------------------
# 
# bs_halflife <- merge_half[,c(1,3:ncol(merge_half) )]
# row.names(bs_halflife) <- bs_halflife$id
# bs_halflife$id <- NULL
# head(bs_halflife)
# #Find  the  0.05  and  0.95  quantile
# bs_quantiles = apply(bs_halflife, 1, quantile, c(0.05,0.95))  %>%t()
# halflife_PbsCI <- merge(separate_half, bs_quantiles, by = 0)
# head(halflife_PbsCI)
# #Add means of bs
# bs_mean <- apply(bs_halflife, 1, mean)
# head(bs_mean)
# halflife_PbsCI <- merge(halflife_PbsCI, bs_mean, by.x = 1,by.y = 0)
# head(halflife_PbsCI)
# #Add intervals
# halflife_PbsCI$interval <- (halflife_PbsCI$`95%`)-(halflife_PbsCI$`5%`)
# head(halflife_PbsCI)
